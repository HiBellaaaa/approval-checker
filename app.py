...
if st.button("送出"):
    if not pay_file or not mac or not date_input:
        ...
        with st.spinner("處理中..."):
            ...
            # 若授權碼為0，顯示 debug 資訊
            if len(auth_codes) == 0:
                st.error("授權碼筆數為 0，可能未正確偵測欄位。")
                try:
                    df_dbg = pd.read_excel(pay_file, header=0)
                    st.write("偵測到的 Excel 欄位:", df_dbg.columns.tolist())
                except Exception:
                    st.write("無法讀取 Excel 進行欄位偵錯。")
