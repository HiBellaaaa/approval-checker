...
log_file = st.file_uploader(
    "2. 上傳 log 檔案（例如：20250703_ui.txt）", type=["txt"]
)
mac = st.text_input("3. 輸入機台 MAC 值（可在販賣機後台機台資訊頁找到）")
date_input = st.date_input("4. 選擇搜尋日期")
time_input = st.time_input("5. 選擇每日最後結帳時間", value=dtime(21, 0))

if st.button("送出"):
    if not pay_file or not log_file or not mac or not date_input:
        st.warning("請確認已上傳對帳檔、上傳 log 檔、輸入 MAC 值及選擇日期與時間後再送出。")
